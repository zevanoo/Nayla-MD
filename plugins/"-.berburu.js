let handler = async (m, { conn, usedPrefix }) => {
    ...
    if (new Date - global.db.data.users[m.sender].lastdungeon > 500000) {
let randomaku1 = `${Math.floor(Math.random() * 10)}`
let randomaku2 = `${Math.floor(Math.random() * 10)}`
let randomaku4 = `${Math.floor(Math.random() * 10)}`
let randomaku3 = `${Math.floor(Math.random() * 10)}`
let randomsword = `${Math.floor(Math.random() * 100)}`
...
global.db.data.users[m.sender].sworddurability -= rbrb
...
handler.tags = ['rpg']
handler.command = /^(memburu|berburu|hunt)$/i
handler.owner = false
handler.mods = false
