...
Ketik ${usedPrefix}susunbantu untuk bantuan
Bonus: ${poin} XP
`.trim()
    conn.susunkata[id] = [
        await conn.sendButton(m.chat, caption, '© nayla', 'Bantuan', '.susunbantu'),
        json, poin,
        setTimeout(async () => {
            if (conn.susunkata[id]) await conn.sendButton(m.chat, `Waktu habis!\nJawabannya adalah *${json.jawaban}*`, '© nayla', 'Susun Kata', '.susunkata')
